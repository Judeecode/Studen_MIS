...
if (is_logged_admin()) header('Location: dashboard.php');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $mysqli->prepare("SELECT id, password, full_name FROM admins WHERE username = ?");
    $stmt->bind_param('s', $u);
    $stmt->execute();
    $res = $stmt->get_result();

    if ($row = $res->fetch_assoc()) {
        if ($row['password'] === md5($p)) { // using MD5 for old PHP
...
<div class="login-container">
  ...
  <div class="logo-section">
    <div class="logo-content">
      ...
      <div class="company-name">Student MIS</div>
    </div>
  </div>
  ...
  <div class="form-section">
    ...
    <div class="form-content">
      <div class="form-header">
        ...
        <p class="subtitle-text">PLEASE LOGIN TO ADMIN DASHBOARD.</p>
      </div>
